...
export function useInteractionImporter() {
  ...
  /**
   * 从单行数据导入交互
   * @param rowData 行数据 [null, target, label, category, content]
   * @returns 是否导入成功
   */
  function importFromRow(rowData: (string | number)[]): boolean {
    try {
      ...
      let content = String(rowData[4] || '').trim();
      ...
      if (playerName) {
        content = content.split(playerName).join('{{user}}');
      }
      ...
      if (target) {
        ...
        content = content.split(target).join('{{rowTitle}}');
      }
      ...
      if (existingTag) {
        ...
        // 创建新标签
        tagStore.createTag(label, categoryId, content);
      }
