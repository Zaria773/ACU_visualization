    ...
    <div ref="gridRef" class="acu-dash-grid">
      ...
      <template v-for="widget in displayWidgets" :key="widget.id">
        ...
        <InteractionTableWidget
          v-if="isInteractionTable(widget)"
          :config="widget"
...
import { DashboardWidget, InteractionTableWidget } from './dashboard';
...
/** 判断是否为交互表格组件 */
function isInteractionTable(widget: DashboardWidgetConfig): boolean {
  // 通过表名判断，或者将来可以在 config 中增加 type
  const table = getTableById(widget.tableId);
  return table?.name === '交互生成器' || widget.tableId === 'sheet_Interaction_CN';
}
